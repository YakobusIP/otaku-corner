...
import AnimeDetailPanel from "@/features/add-media/components/add-anime/AnimeDetailPanel";
...
import { useAddAnimeDialog } from "@/features/add-media/hooks/useAddAnimeDialog";
import { TENRAI_SEARCH_MIN_LENGTH } from "@/features/add-media/hooks/useTenraiSelection";

import { displayYearAnime, posterUrl } from "@/features/add-media/lib/media-dialog-helpers";
...
export default function AddAnimeDialog({
  ...
  const selectedRows = selectedItems.map((anime) => (
    <SelectedRow
      ...
      mediaType="anime"
      malId={anime.mal_id}
      titlePrimary={anime.title_english || anime.title}
      titleSecondary={anime.title_japanese || anime.title}
      typeLabel={anime.type ?? "-"}
      yearBadge={displayYearAnime(anime)}
      posterSrc={posterUrl(anime)}
      removeAriaTitle={anime.title}
