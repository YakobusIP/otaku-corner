...
import SelectedRow from "@/features/add-media/components/SelectedRow";
import MangaDetailPanel from "@/features/add-media/components/add-manga/MangaDetailPanel";
...
import { useAddMangaDialog } from "@/features/add-media/hooks/useAddMangaDialog";
import { TENRAI_SEARCH_MIN_LENGTH } from "@/features/add-media/hooks/useTenraiSelection";

import {
  displayYearFromPublished,
  posterUrl
} from "@/features/add-media/lib/media-dialog-helpers";
...
export default function AddMangaDialog({
  ...
  const selectedRows = selectedItems.map((manga) => (
    <SelectedRow
      ...
      mediaType="manga"
      malId={manga.mal_id}
      titlePrimary={manga.title_english || manga.title}
      titleSecondary={manga.title_japanese || manga.title}
      typeLabel={manga.type ?? "-"}
      yearBadge={displayYearFromPublished(manga)}
      posterSrc={posterUrl(manga)}
      removeAriaTitle={manga.title}
      isActive={activeDetailId === manga.mal_id}
      onDuplicateStatus={handleDuplicateStatus}
      onPick={() =>
        setActiveDetailId((id) => {
          const next = id === manga.mal_id ? null : manga.mal_id;
          setDetailOpen(next !== null);
          return next;
        })
      }
      onRemove={() => removeSelected(manga.mal_id)}
    />
  ));
    ...
    <Dialog open={openDialog} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader className="shrink-0 space-y-1 border-b border-border/40 px-6 py-4 text-left">
          ...
          <DialogDescription>
            Search and select manga to add to your collection
          </DialogDescription>
        </DialogHeader>
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1 flex flex-wrap gap-1">
                                ...
                                <Badge
                                  ...
                                  className="text-[10px]"
                                >
                                  {displayYearFromPublished(manga)}
                                </Badge>
                              </div>
                            </div>
                            <Button
                              ...
                              onClick={() => addFromSearch(manga)}
                              aria-label={
                                alreadySelected
                                  ? `${manga.title_english || manga.title} already in selection`
                                  : `Add ${manga.title_english || manga.title}`
                              }
                            >
                              {alreadySelected ? (
                  ...
                  {!activeDetail ? (
                    ...
                    <div className="flex min-h-0 flex-1 flex-col overflow-hidden border-t border-border/40">
                      ...
                      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3 pb-3 pt-3">
                        <MangaDetailPanel manga={activeDetail} />
                      </div>
                    </div>
                  )}
                </div>
