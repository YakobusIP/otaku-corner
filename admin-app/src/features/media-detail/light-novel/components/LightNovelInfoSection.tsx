import EditVolumesModal from "@/features/media-detail/light-novel/components/EditVolumesModal";
...
export default function LightNovelInfoSection({
  lightNovelDetail,
  resetParent
}: Props) {
  const infoItems = [
    ...
    },
    {
      key: "published",
      label: "Published",
      value: lightNovelDetail.published || "—",
      icon: CalendarIcon,
      accent: "text-sky-300"
    },
    {
  ...
  const entityGroups = [
    {
      ...
      items: lightNovelDetail.authors.map((a) => ({ id: a.id, name: a.name }))
    },
    {
      key: "genres",
      label: "Genres",
      ...
      items: lightNovelDetail.genres.map((g) => ({ id: g.id, name: g.name }))
    },
    {
      key: "themes",
      label: "Themes",
      ...
      items: lightNovelDetail.themes.map((t) => ({ id: t.id, name: t.name }))
    }
  ];
