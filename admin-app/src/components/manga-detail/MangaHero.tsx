...
export default function MangaHero({ mangaDetail }: Props) {
  const cover =
    mangaDetail.images.large_image_url ?? mangaDetail.images.image_url;

  const personalScore = mangaDetail.review?.personalScore;

  const status =
    STATUS_THEMES[mangaDetail.review.progressStatus] ?? STATUS_THEMES.PLANNED;
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <StatPill
              ...
              label="MAL"
              value={mangaDetail.score ? mangaDetail.score.toFixed(2) : "N/A"}
            />
            <StatPill
            ...
            />
            <StatPill
              icon={<HashIcon className="h-4 w-4 text-sky-400" />}
              label="Chapters"
              value={mangaDetail.chaptersCount?.toString() ?? "—"}
            />
            <StatPill
              icon={<BookCopyIcon className="h-4 w-4 text-fuchsia-400" />}
              label="Volumes"
