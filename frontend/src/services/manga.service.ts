...
const fetchAllMangaService = async (
  ...
  try {
    const response = await axios.get(BASE_MANGA_URL, {
      params: {
        currentPage,
        limitPerPage,
        q: query,
...
const addMangaService = async (
  ...
  try {
    const response = await axios.post(BASE_MANGA_URL, data);
...
const updateMangaReviewService = async (
  ...
  try {
    const response = await axios.put(`${BASE_MANGA_URL}/review/${id}`, data);
    return { success: true, data: response.data };
  } catch (error) {
    return {
      success: false,
...
const deleteMangaService = async (
  ...
  try {
    await axios.delete(BASE_MANGA_URL, { data: { ids } });
