...
export default function AnimeCard({ anime }: Props) {
  const yearLabel = anime.season
    ? anime.season.slice(-4)
    : anime.aired
      ? anime.aired.slice(-4)
      : null;
  ...
  const malScoreBadge = (variant: "overlay" | "inline") => (
    <ScoreBadge
      ...
      value={formatScoreFixedOrNa(anime.score)}
    />
  );
  ...
  const personalScoreBadge = (variant: "overlay" | "inline") =>
    anime.personalScore != null ? (
      <ScoreBadge
        ...
        value={formatScoreFixedOrNa(anime.personalScore)}
      />
    ) : null;
      ...
      >
        <div className="relative w-28 shrink-0 md:w-full">
          ...
          <div className="absolute top-2 left-2 md:hidden">
            <ProgressStatusPill progressStatus={anime.progressStatus} />
          </div>
