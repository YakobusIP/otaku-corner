...
export class LightNovelService {
  ...
  async getLightNovelStatusCounts() {
    try {
      const totalCount = await this.getTotalData();

      const statusCounts = await prisma.lightNovelReview.groupBy({
        by: ["progressStatus"],
        _count: true
      });
      ...
      const mappedStatus = Object.keys(LightNovelService.progressStatuses).map(
        (status) => ({
          label: LightNovelService.progressStatuses[status],
          value: status,
