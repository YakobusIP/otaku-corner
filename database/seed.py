...
async def create_manga(session, manga_data):
    ...
    manga_payload = {
        ...
        "titleSynonyms": " ".join([synonym.lower() for synonym in manga_data["title_synonyms"]]) if manga_data["title_synonyms"] else "",
        "published": (f"{datetime.strptime(manga_data['published']['from'], '%Y-%m-%dT%H:%M:%S%z').strftime('%b %d, %Y')} to "
                      f"{datetime.strptime(manga_data['published']['to'], '%Y-%m-%dT%H:%M:%S%z').strftime('%b %d, %Y') if manga_data['published']['to'] else '?'}")
                     if manga_data["status"] != "Upcoming" else manga_data["status"],
        "chaptersCount": manga_data["chapters"],
        "volumesCount": manga_data["volumes"],
        "score": manga_data["score"],
        "images": {
            "image_url": manga_data["images"]["webp"]["image_url"] if manga_data["images"]["webp"] else manga_data["images"]["jpg"]["image_url"],
            "large_image_url": manga_data["images"]["webp"]["large_image_url"] if manga_data["images"]["webp"] else manga_data["images"]["jpg"]["large_image_url"],
            "small_image_url": manga_data["images"]["webp"]["small_image_url"] if manga_data["images"]["webp"] else manga_data["images"]["jpg"]["small_image_url"]
        },
    ...
    try:
        async with session.post(backend_url, json=manga_payload) as response:
            if response.status == 201:
                console.print(f"Manga [bold]{manga_data['title']}[/bold] created successfully!", style="green")
                success_count += 1
            else:
                console.print(f"Failed to create manga {manga_data['title']}: {response.status}", style="red")
                failure_count += 1
                failed_ids.append(manga_data["mal_id"])
                pprint(manga_payload)
    except Exception as e:
        console.print(f"Error while creating anime {manga_data['title']}: {str(e)}", style="red")
        failure_count += 1
        failed_ids.append(manga_data["mal_id"])
